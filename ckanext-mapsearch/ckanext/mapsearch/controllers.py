...
from ckanext.spatial.lib import bbox_query,validate_bbox
...
class ViewController(BaseController):
    ...
    def text_complete(self):
        ...
        return json.dumps([r["title"] for r in text_results['results'] if r['id'] in geo_results['results']])

    def query_datasets(self):
        """the full query returning datasets"""
        text_results, geo_results = self._do_query(request)
        return json.dumps([r for r in text_results['results'] if r['id'] in geo_results['results']])
